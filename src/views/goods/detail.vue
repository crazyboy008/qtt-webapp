...
<!--          <van-swipe-item>
          ...
          <div class="custom-indicator" slot="indicator">
            {{ current + 1 }}/{{data.header_imgs.length}}
          </div>
        </van-swipe>
      </div>
      ...
      <div class="coupon-wrap">
        ...
        <div class="coupon" @click="goBuy">
          <div class="icon">
            <img src='@/assets/images/product-quan.png' alt=""/>
          </div>
          <div class="main">
            <p>优惠券</p>
            <p>有效期：{{coupon.coupon_start_date}} - {{coupon.coupon_end_date}}</p>
          </div>
          <div class="amount">
      ...
      <div class="shop">
        ...
        <a :href="shop.shop_url" class="button">
          <span>进入店铺</span>
        </a>
      </div>
      <div class="describe">
        <p>
          <span>宝贝描述：&nbsp;</span>
          <span>{{shop.item_score}}</span>
        </p>
        <p>
          <span>卖家服务：&nbsp;</span>
          <span>{{shop.service_score}}</span>
        </p>
        <p>
          <span>物流服务：&nbsp;</span>
          <span>{{shop.delivery_score}}</span>
        </p>
      </div>
      <div class="img-box">
      ...
      <van-goods-action>
        ...
        <van-goods-action-button type="warning" :text="`券后价 ¥${data.price}`" />
        <van-goods-action-button type="danger" text="立即领券购买" id="code" :data-clipboard-text="copy_url" @click="goBuy" />
      </van-goods-action>
    </div>
...
<script>
  ...
  import {getGoodsDetail, urlGenerate, getTbGoodsDetail} from '@/assets/js/api'
  ...
  export default {
    ...
    methods: {
      ...
      goBuy() {
        if (this.copy_url) window.location.href = this.copy_url
        // this.show = true
      },
      onChange(index) {
        this.current = index;
      ...
      async _getGoodsDetail() {
        let res = await getGoodsDetail({goods_id_list: `[${this.$route.params.id}]`})
        this.data = res
        // this.videos = res.videos
        this.shop = res.shop
        this.coupon = res.coupon
        this.loading = false
        this._geturlGenerate()
      },
      async _geturlGenerate() {
        let res = await urlGenerate({goods_id_list: `[${this.data.source_id}]`})
        this.copy_url = res.data.mobile_short_url
        console.log(res)
      },
      initCopy() {
        ...
        this.clipboard.on('success', (e) => {
          e.clearSelection()
          // this.$toast('淘口令复制成功，立即打开手淘APP领券')
